...
final class StackdriverExportUtils {
  ...
  @VisibleForTesting static final String NAMESPACE = "custom_metrics";
  ...
  static MetricDescriptor createMetricDescriptor(View view, String projectId) {
    if (!(view.getWindow() instanceof Cumulative)) {
      // TODO(songya): Only Cumulative view will be exported to Stackdriver in v0.1
      return null;
    }

    MetricDescriptor.Builder builder = MetricDescriptor.newBuilder();
    String name = view.getName().asString();
    builder.setName(createMetricDescriptorName(projectId, name));
    builder.setType(createMetricType(name));
    builder.setDescription(view.getDescription());
    builder.setUnit(view.getMeasure().getUnit());
    // TODO(songya): do we want a different display name?
    builder.setDisplayName(NAMESPACE + "/" + name);
  ...
  // Construct a name for a MetricDescriptor
  // Name format refers to cloud.google.com/monitoring/api/ref_v3/rest/v3/projects.metricDescriptors
  private static String createMetricDescriptorName(
      String projectId, String viewName) {
    return String.format(
        "projects/%s/metricDescriptors/custom.googleapis.com%%2Fcensus%%2F%s%%2F%s",
        projectId, NAMESPACE, viewName);
  }

  // Construct a MetricDescriptor.Type. From the documentation:
  // cloud.google.com/monitoring/api/ref_v3/rest/v3/projects.metricDescriptors
  // For now only support custom metrics.
  private static String createMetricType(String viewName) {
    return String.format("custom.googleapis.com/census/%s/%s", NAMESPACE, viewName);
  }
  ...
  static MetricKind createMetricKind(AggregationWindow window) {
    return window.match(
        ...
        // We don't support exporting Interval stats to StackDriver in v0.1
  ...
  static Metric createMetric(View view, List<? extends TagValue> tagValues) {
    ...
    builder.setType(createMetricType(view.getName().asString()));
    ...
    windowData.match(
        ...
        new Function<IntervalData, Void>() {
          ...
          public Void apply(IntervalData arg) {
            // TODO(songya): we don't export IntervalData in v0.1
            return null;
          }
        },
