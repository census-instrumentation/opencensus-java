...
public class MutableAggregationTest {
  ...
  public void testAdd() {
    ...
    assertAggregationDataEquals(
        ...
        AggregationData.DistributionData.create(
            8.666666, 3, 1.0, 20.0, 200.666666, Arrays.asList(0L, 2L, 1L)),
        TOLERANCE);
    assertAggregationDataEquals(
  ...
  public void testCombine_Distribution() {
    ...
    verifyMutableDistribution(combined, 5, 1, 5, 5, 0.0, new long[] {0, 1, 0}, TOLERANCE);

    combined.combine(distribution2, 1.0); // distribution2 will be combined
    verifyMutableDistribution(
        combined, 11.666666, 3, 5, 20, 116.66666666, new long[] {0, 1, 2}, TOLERANCE);

    combined.combine(distribution3, 1.0); // distribution3 will be combined
    verifyMutableDistribution(combined, 12.5, 4, 5, 20, 125.0, new long[] {0, 1, 3}, TOLERANCE);
  }
